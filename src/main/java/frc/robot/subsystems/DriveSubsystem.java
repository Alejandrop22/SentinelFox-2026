...
public class DriveSubsystem extends SubsystemBase {
  ...
  private void updateAutoAimRotation() {
    ...
    if (dist > kMinLeadDistanceMeters && dist <= kMaxLeadDistanceMeters) {
      ...
      if (dt > 1e-3 && dt < 0.25) {
        m_estLatAccelMps2 = (vLat - m_prevVyMps) / dt;
      }
      m_prevVyMps = vLat;
    ...
    SmartDashboard.putNumber("AutoAim/VLatMps", m_lastChassisSpeeds.vyMetersPerSecond);
    SmartDashboard.putNumber("AutoAim/ALatMps2", m_estLatAccelMps2);
  ...
  /**
   * Crea un comando que mueve al robot una distancia traslacional en coordenadas del CAMPO
   * (field-relative), sin girar (rot=0). Usa odometria (Pose2d) para parar cuando llega.
   *
   * @param dxMeters +X del campo (meters). Campo-X normalmente es "hacia adelante" de la alianza azul.
   * @param dyMeters +Y del campo (meters).
   * @param speedMps magnitud de velocidad (m/s). Se limita por kMaxSpeedMetersPerSecond.
   */
  ...
  /**
   * Igual que driveFieldRelativeDistance, pero mientras se mueve gira automáticamente a un heading objetivo
   * (en grados, coordenadas de campo). Es ideal para autónomo.
   *
   * targetHeadingDeg: 0 = "norte" del campo (si tu gyro está reseteado apuntando al norte), 90 = este, etc.
   */
